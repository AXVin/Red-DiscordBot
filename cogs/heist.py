...
class Heist:
    ...
    @pubg.command(name="themes", pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    async def _themelist_heist(self):
        """Lists available themes for the game."""
    ...
    @pubg.command(name="reset", pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    async def _reset_heist(self, ctx):
        """Resets game in case it hangs"""
        server = ctx.message.server
        settings = self.check_server_settings(server)
        self.reset_heist(settings)
        await self.bot.say("```Game has been reset```")

    @pubg.command(name="clear", pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    async def _clear_heist(self, ctx, user: discord.Member):
        """Clears a member of knock and death statuses."""
        author = ctx.message.author
        settings = self.check_server_settings(author.server)
        self.user_clear(settings, user)
        await self.bot.say("```{} administratively cleared {}```".format(author.name, user.name))

    @pubg.command(name="version", pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    async def _version_heist(self):
        """Shows the version of PUBGD you are running"""
        await self.bot.say("You are running PUBGD version {}.".format(self.version))

    @pubg.command(name="locations", pass_context=True)
    async def _targets_heist(self, ctx):
        ...
        if len(settings["Targets"].keys()) < 0:
            msg = ("There aren't any locations! To create a target use {}heist "
                   "createlocation .".format(ctx.prefix))
        else:
    ...
    @pubg.command(name="payout", pass_context=True)
    async def _bailout_heist(self, ctx, user: discord.Member=None):
        """Specify who you want to pay for revive. Defaults to you."""
        author = ctx.message.author
        settings = self.check_server_settings(author.server)
        ...
        if settings["Players"][player.id]["Status"] != "Apprehended":
            return await self.bot.say("{} is not knocked off!".format(player.name))
        ...
        if player.id == author.id:
            msg = ("Do you want to make a {0} amount? It will cost {1} credits. If you are "
                   "knocked again, your next {2} and {0} amount will triple. "
                   "Do you still wish to pay the {0} amount?".format(t_bail, cost, t_sentence))
        else:
            msg = ("You are about to pay a {2} amount for {0} and it will cost you {1} credits. "
        ...
        if response.content.title() == "Yes":
            msg = ("Congratulations {}, you are revived! Enjoy your life while it "
    ...
    @pubg.command(name="createlocation", pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    async def _targetadd_heist(self, ctx):
        """Add a location to game"""
        ...
        start = ("This will walk-through the location creation process. You may cancel this process "
                 "at anytime by typing {}cancel. Let's begin with the first question.\nWhat is the "
                 "name of this location?".format(ctx.prefix))

        await self.bot.say(start)
        name = await self.bot.wait_for_message(timeout=35, author=author)

        if name is None:
            await self.bot.say("You took too long. Canceling location creation.")
            return

        if name.content == cancel:
            await self.bot.say("Location creation cancelled.")
            return

        if string.capwords(name.content) in settings["Targets"]:
            await self.bot.say("A location with that name already exists. Canceling location "
                               "creation.")
            return
        ...
        if int(crew.content) in [subdict["Crew"] for subdict in settings["Targets"].values()]:
            await self.bot.say("Group size conflicts with another location. Canceling location "
                               "creation.")
            return

        await self.bot.say("How many starting credits does this location have?")
        ...
        await self.bot.say("What is the maximum number of credits this location can hold?")
        ...
        await self.bot.say("What is the individual chance of success for this location? 1-100")
    ...
    @pubg.command(name="editlocation", pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    async def _edittarget_heist(self, ctx, *, target: str):
        """Edits the information about a location"""
        author = ctx.message.author
        settings = self.check_server_settings(author.server)
        target = string.capwords(target)

        if target not in settings["Targets"]:
            return await self.bot.say("That location does not exist.")
        ...
        if response.content.title() == "Name":
            await self.bot.say("What would you like to rename the location to?\n*Cannot be a name "
                               "currently in use.*")
            ...
            await self.bot.say("What would you like to change the max crew size to?\n Cannot be "
                               "the same as another location and will be the maximum number of "
                               "players for that location.")
    ...
    @pubg.command(name="remove", pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    async def _remove_heist(self, ctx, *, target: str):
        ...
        if string.capwords(target) in settings["Targets"]:
            await self.bot.say("Are you sure you want to remove {} from the list of "
                               "locations?".format(string.capwords(target)))
            response = await self.bot.wait_for_message(timeout=15, author=author)
            if response is None:
                ...
                self.save_system()
                msg = "{} was removed from the list of locations.".format(string.capwords(target))
            else:
                msg = "Canceling location removal."
        else:
            msg = "That location does not exist."
        await self.bot.say(msg)

    @pubg.command(name="info", pass_context=True)
    async def _info_heist(self, ctx):
        """Shows the PUBGD settings for this server."""
        server = ctx.message.server
        settings = self.check_server_settings(server)
        ...
        description = ["PUBGD Version {}".format(self.version), "Theme: {}".format(theme)]
        footer = "PUBGD was inspired from Heists by Redjumpman. Created by AXVin#2130"

        embed = discord.Embed(colour=0x0066FF, description="\n".join(description))
        embed.title = "{} PUBGD Settings".format(server.name)
        embed.add_field(name="Game Cost", value=settings["Config"]["Heist Cost"])
    ...
    @pubg.command(name="revive", pass_context=True)
    async def _release_heist(self, ctx):
        """Removes you from jail or clears revive status if sentence served."""
        author = ctx.message.author
        server = ctx.message.server
        settings = self.check_server_settings(server)
        self.account_check(settings, author)
    ...
    @pubg.command(name="rise", pass_context=True)
    async def _revive_heist(self, ctx):
        ...
        if settings["Players"][author.id]["Status"] == "Dead":
            ...
            if remainder == "No Cooldown":
                ...
                msg = ("You can't rise yet. You still need to wait:\n"
                       "```{}```".format(remainder))
        else:
            msg = "You still have a pulse. I can't revive someone who isn't dead."
        await self.bot.say(msg)

    @pubg.command(name="stats", pass_context=True)
    async def _stats_heist(self, ctx):
        """Shows your PUBGD stats"""
        author = ctx.message.author
        ...
        embed.add_field(name="Revived", value=path["Jail Counter"])
        embed.add_field(name="Death Timer", value=death_fmt)
        embed.add_field(name="Total Deaths", value=path["Deaths"])
        embed.add_field(name="Lifetime Revives", value=path["Total Jail"])

        await self.bot.say(embed=embed)

    @pubg.command(name="play", pass_context=True)
    async def _play_heist(self, ctx):
        ...
        if not settings["Config"]["Heist Planned"]:
            ...
            await self.bot.say("A {4} is being planned by {0}\nThe {4} "
                               "will begin in {1} seconds. Type `{2}pubg play` to join their "
    ...
    @commands.group(pass_context=True, no_pm=True)
    async def setpubg(self, ctx):
        """Set different options in the pubg config"""

        if ctx.invoked_subcommand is None:
            await send_cmd_help(ctx)

    @pubg.command(name="theme", pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    async def _theme_heist(self, ctx, theme):
        """Sets the theme for PUBG"""
        theme = theme.title()
        server = ctx.message.server
        settings = self.check_server_settings(server)
    ...
    @setpubg.command(name="output", pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    ...
    @setpubg.command(name="revivetime", pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    async def _sentence_setheist(self, ctx, seconds: int):
        """Set the base revive time when caught"""
        server = ctx.message.server
        settings = self.check_server_settings(server)
        t_jail = settings["Theme"]["Jail"]
        t_sentence = settings["Theme"]["Sentence"]
    ...
    @setpubg.command(name="cost", pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    async def _cost_setheist(self, ctx, cost: int):
        """Set the cost to play PUBGD"""
        server = ctx.message.server
        settings = self.check_server_settings(server)
        ...
        if cost >= 0:
            ...
            self.save_system()
            msg = "Setting PUBGD cost to {}.".format(cost)
        else:
            msg = "Need a number higher than -1."
        await self.bot.say(msg)

    @setpubg.command(name="authorities", pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    ...
    @setpubg.command(name="revivecost", pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    async def _bail_setheist(self, ctx, cost: int):
        """Set the cost of revive"""
        server = ctx.message.server
        settings = self.check_server_settings(server)
        t_bail = settings["Theme"]["Bail"]
        if cost >= 0:
    ...
    @setpubg.command(name="death", pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    ...
    @setpubg.command(name="hardcore", pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    ...
    @setpubg.command(name="wait", pass_context=True)
    @checks.admin_or_permissions(manage_server=True)
    ...
    def theme_loader(self, settings, theme_name):
        ...
        if all(key in theme for key in keys):
            ...
            self.save_system()
            return "{} theme found. PUBGD will now use this for future games.".format(theme_name)
        else:
    ...
    def requirement_check(self, settings, prefix, author, cost):
        ...
        if not list(settings["Targets"]):
            msg = ("Oh no! There are no locations! To start creating a location, use "
                   "{}pubg createlocation.".format(prefix))
            return "Failed", msg
        elif settings["Config"]["Heist Start"]:
            msg = ("An {0} is already underway. Wait for the current one to "
            ...
            if remaining == "No Cooldown":
                msg = ("Looks like your {} is over, but you're still in {}! Get "
                       "released by typing {}pubg revive .".format(t_sentence, t_jail, prefix))
            else:
            ...
            msg = ("The {} are on high alert after the last location. We should "
                   "wait for things to cool off before hitting another location.\n"
    ...
    def criminal_level(self, level):
        status = ["Bronze", "Silver", "Gold", "Platinum", "Diamond", "Crown",
                  "Ace"]
    ...
    def patch_2220(self, path):
        ...
        if "Theme" not in path or not path["Theme"]:
            path["Theme"] = {"Jail": "reviving", "OOB": "running on painkillers", "Police": "Enemy Squads",
                             "Bail": "revive", "Crew": "squad", "Sentence": "nap",
                             "Heist": "attack", "Vault": "City"},
        if "Banks" in path:
    ...
    def check_server_settings(self, server):
        if server.id not in self.system["Servers"]:
            default = {"Config": {"Heist Start": False, "Heist Planned": False, "Heist Cost": 100,
                                  ...
                                  "Death Timer": 86400, "Theme": "Pubg", "Crew Output": "None",
                                  "Version": 2.41},
                       "Theme": {"Jail": "reviving", "OOB": "running on painkillers", "Police": "Enemy Squads",
                                 "Bail": "revive", "Crew": "squad", "Sentence": "nap",
                                 "Heist": "attack", "Vault": "City"},
                       "Players": {},
                       "Crew": {},
            ...
            self.save_system()
            print("Creating PUBGD settings for Server: {}".format(server.name))
